"""
Daily Quest management endpoints for School ADMIN.
"""
import logging
...
router = APIRouter(prefix="/daily-quests", tags=["Daily Quests (School)"])
...
async def list_school_daily_quests(
    current_user: User = Depends(require_admin),
    school_id: int = Depends(get_current_user_school_id),
    db: AsyncSession = Depends(get_db),
):
    """List global (read-only) + school-specific daily quests."""
    repo = GamificationRepository(db)
    quests = await repo.list_daily_quests(school_id=school_id)
    return [_to_admin_response(q) for q in quests]


@router.post("", response_model=DailyQuestAdminResponse)
async def create_school_daily_quest(
    data: DailyQuestCreate,
    current_user: User = Depends(require_admin),
    school_id: int = Depends(get_current_user_school_id),
    db: AsyncSession = Depends(get_db),
):
    """Create a school-specific daily quest."""
    # Validate quest_type
    try:
    ...
    if data.textbook_id:
        result = await db.execute(
            select(Textbook).where(
                Textbook.id == data.textbook_id,
                (Textbook.school_id.is_(None)) | (Textbook.school_id == school_id),
            )
        )
        if not result.scalar_one_or_none():
            raise HTTPException(400, f"Textbook {data.textbook_id} not found or not accessible")
    ...
    quest = DailyQuest(
        ...
        school_id=school_id,  # School-specific
...
async def get_school_daily_quest(
    quest_id: int,
    current_user: User = Depends(require_admin),
    school_id: int = Depends(get_current_user_school_id),
    db: AsyncSession = Depends(get_db),
):
    """Get a single daily quest (own school or global)."""
    repo = GamificationRepository(db)
    quest = await repo.get_daily_quest_by_id(quest_id)
    if not quest:
        raise HTTPException(404, "Daily quest not found")
    # School admin can see global or own school quests
    if quest.school_id is not None and quest.school_id != school_id:
        raise HTTPException(403, "Access denied")
    return _to_admin_response(quest)


@router.put("/{quest_id}", response_model=DailyQuestAdminResponse)
async def update_school_daily_quest(
    quest_id: int,
    ...
    """Update a school-specific daily quest (own school only)."""
    repo = GamificationRepository(db)
    quest = await repo.get_daily_quest_by_id(quest_id)
    if not quest:
        raise HTTPException(404, "Daily quest not found")
    if quest.school_id is None:
        raise HTTPException(403, "Cannot edit global quests")
    if quest.school_id != school_id:
        raise HTTPException(403, "Access denied")
    ...
    if 'textbook_id' in update_data and update_data['textbook_id']:
        result = await db.execute(
            select(Textbook).where(
                Textbook.id == update_data['textbook_id'],
                (Textbook.school_id.is_(None)) | (Textbook.school_id == school_id),
            )
        )
        if not result.scalar_one_or_none():
            raise HTTPException(400, f"Textbook {update_data['textbook_id']} not found or not accessible")
...
async def delete_school_daily_quest(
    quest_id: int,
    current_user: User = Depends(require_admin),
    school_id: int = Depends(get_current_user_school_id),
    db: AsyncSession = Depends(get_db),
):
    """Deactivate a school-specific daily quest (own school only)."""
    repo = GamificationRepository(db)
    quest = await repo.get_daily_quest_by_id(quest_id)
    if not quest:
        raise HTTPException(404, "Daily quest not found")
    if quest.school_id is None:
        raise HTTPException(403, "Cannot delete global quests")
    if quest.school_id != school_id:
        raise HTTPException(403, "Access denied")
