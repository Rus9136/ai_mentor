"""
LLM Usage monitoring endpoints for School ADMIN.
"""
...
router = APIRouter(prefix="/llm-usage", tags=["LLM Usage (School)"])
...
async def get_school_stats(
    ...
    """Get LLM usage summary for the school."""
    ...
    ).where(*filters).group_by(LLMUsageLog.feature)
    ...
    model_q = select(
        ...
        func.count().label("total_calls"),
        func.coalesce(func.sum(LLMUsageLog.total_tokens), 0).label("total_tokens"),
    ).where(*filters).group_by(LLMUsageLog.model, LLMUsageLog.provider)
...
async def get_school_daily_stats(
    date_from: Optional[date] = Query(None),
    date_to: Optional[date] = Query(None),
    current_user: User = Depends(require_admin),
    school_id: int = Depends(get_current_user_school_id),
    ...
    """Get daily LLM usage for the school."""
    dt_from, dt_to = _default_date_range(date_from, date_to)
