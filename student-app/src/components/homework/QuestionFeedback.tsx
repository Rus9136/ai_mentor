...
import { CheckCircle2, XCircle, AlertTriangle, Brain, FileCheck } from 'lucide-react';
import { SubmissionResult, TaskType } from '@/lib/api/homework';
import { cn } from '@/lib/utils';

// Task types that don't show correct/incorrect status (AI-graded open tasks)
...
export function QuestionFeedback({
  ...
  // Open task types show different UI (no correct/incorrect, just AI feedback)
  const isOpenTask = taskType && OPEN_TASK_TYPES.includes(taskType);

  // For open tasks, render a different card style
  if (isOpenTask) {
    return (
      <div
        className={cn(
          'rounded-xl p-4 border',
          isPending ? 'bg-amber-50 border-amber-200' : 'bg-blue-50 border-blue-200'
        )}
      >
        {/* Status Header for open tasks */}
        <div className="flex items-center gap-2 mb-2">
          {isPending ? (
            ...
            <>
              ...
              <span className="font-semibold text-blue-700">{t('evaluated')}</span>
            </>
          )}
        ...
        {/* AI Feedback - main focus for open tasks */}
  ...
  // Original UI for quiz/test tasks
  return (
    <div
      className={cn(
        'rounded-xl p-4 border',
