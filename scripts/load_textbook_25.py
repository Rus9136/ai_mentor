#!/usr/bin/env python3
"""
Load 'Қазақстан тарихы 7' (Kazakh) into the AI Mentor database.

Parses the Mathpix MMD file, creates textbook/chapter/paragraph records,
and creates ParagraphContent records for language='kk'.

This is a HISTORY textbook — no exercises, no A/B/C difficulty levels.
Chapters are determined by § number ranges from the TOC.

Usage:
    python scripts/load_textbook_25.py --dry-run           # Parse only
    python scripts/load_textbook_25.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_25.py --update-content FILE # Update SQL
"""
import re
import sys
import os
...
TEXTBOOK_ID = 25  # Already exists in DB
TEXTBOOK_TITLE = "Қазақстан тарихы"
SUBJECT_CODE = "history_kz"
GRADE_LEVEL = 7
AUTHORS = "Т.Омарбеков, Г.Б.Хабижанова, Т.Е.Қартаева, М.С.Ноғайбаева, Г.Е.Абикенова"
PUBLISHER = "Мектеп"
YEAR = 2025
ISBN = "978-601-07-1768-9"
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "25" / "textbook_25.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "25"
...
CHAPTERS = [
    {
        "number": 1,
        "title": "І бөлім. ҚАЗАҚ-ЖОҢҒАР СОҒЫСТАРЫ",
        "para_from": 1,
        "para_to": 6,
    },
    {
        "number": 2,
        "title": "ІІ бөлім. XVIII ҒАСЫРДАҒЫ ҚАЗАҚ ХАНДЫҒЫ",
        "para_from": 7,
        "para_to": 14,
    },
    {
        "number": 3,
        "title": "ІІІ бөлім. XVIII ҒАСЫРДАҒЫ ҚАЗАҚСТАННЫҢ МӘДЕНИЕТІ",
        "para_from": 15,
        "para_to": 18,
    },
    {
        "number": 4,
        "title": "IV бөлім. ОТАРЛАУ ЖӘНЕ ҰЛТ-АЗАТТЫҚ КҮРЕС",
        "para_from": 19,
        "para_to": 32,
    },
    {
        "number": 5,
        "title": "V бөлім. ҚАЗАҚСТАН РЕСЕЙ ИМПЕРИЯСЫНЫҢ ҚҰРАМЫНДА",
        "para_from": 33,
        "para_to": 45,
    },
    {
        "number": 6,
        "title": "VI бөлім. XIX ҒАСЫР — XX ҒАСЫРДЫҢ БАСЫНДАҒЫ ҚАЗАҚСТАННЫҢ МӘДЕНИЕТІ",
        "para_from": 46,
        "para_to": 60,
    },
]
...
INTERNAL_HEADINGS = {
    # Lesson structure
    "Бул сабақта:", "Бұл сабақта:", "эфл сабақта:",
    "Тірек сөздер",
    "Білімдеріңді шыңдаңдар:", "Білімдеріңді шыңдацдар:", "Білімдеріңді шыңдандар:",
    # Preamble/meta
    "АЛҒЫ СОЗ",
    "Авторлары:",
    "Шартты белгілер:",
    "- 1 - бұл қызық",
    # Chapter-level headings (not paragraph boundaries, handled by § mapping)
    "ҚАЗАҚ-ЖОҢғАР СОҒЫСТАРЫ",
    "VI бөлім",
    "XIX FACIP - XX FACIPДЫҢ",
    "БАСЫНДАҒЫ ҚАЗАҚСТАННЫҢ",
    "МӘДЕНИЕТІ",
    # Person names inside paragraphs (§6)
    "Төле би Әлібекұлы",
    "Қазыбек би Келдібекұлы",
    "Әйтеке би Бәйбекұлы",
    # Map legends
    "ШЕГАРАЛАР:", "АУМАҚТАР:",
    "- - - мемлекеттік (болжамды)",
    # End-of-book sections (stop markers)
    "БАСТЫ ОҚИҒАЛАР",
    "ТЕРМИНДЕР СӨЗДІГІ",
    ...
    "Все учебники Казахстана ищите на сайтах OKULYK.COM и OKULYK.K4",
    "Все учебники Казахстана ищите на сайтах OKULYK.COM и OKULYK.Kł",
    # OCR junk
    "回剖回",
    # TOC entries that appear as headings
    "V бөлім. ҚАЗАҚСТАН РЕСЕЙ ИМПЕРИЯСЫНЫН ҚҮРАМЫНДА",
}
...
class ParsedParagraph:
    title: str
    number: int      # First § number (e.g., 7 for "§ 7-8")
    raw_number: str   # Original string (e.g., "7-8")
...
RE_STOP = re.compile(
    r"^#{1,2}\s+(БАСТЫ ОҚИҒАЛАР|ТЕРМИНДЕР СӨЗДІГІ|ПАЙДАЛАНЫЛҒАН|МАЗМУНЫ)",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    # Heading starting with 0 (icon marker like "0 Қорқыттың...")
    ...
    # Author names that leaked as headings
    if clean.startswith("Т.Омарбеков") or clean.startswith("Т. Омарбеков"):
        return True

    # OCR junk
    if any(ord(c) > 0x3000 for c in clean):  # CJK characters
        return True
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if in_abstract:
            if line.strip() == "" or line.startswith("#"):
                ...
                # Abstract content — if we have a current paragraph,
                # add the abstract content as regular content
        ...
        if m:
            raw_num = m.group(1)  # e.g., "7-8", "19-20", "1"
...
def md_lines_to_html(lines: list[str], textbook_id: int) -> str:
    ...
    for raw_line in content.split("\n"):
        ...
        # Task line: "N. Text" (numbered items in exercises)
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Қазақстан тарихы 7 Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_25.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Қазақстан тарихы 7 Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Қазақстан тарихы 7 textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Қазақстан тарихы 7 — Textbook Import")
    print("=" * 70)
