#!/usr/bin/env python3
"""
Load 'Қазақстан тарихы 11 сынып, 2-бөлім' into the AI Mentor database.
...
Chapters are determined by § number ranges (compound §§: §23-24, §25-26, etc.).
The book has 6 chapters (тарау 6-11) organized in 2 internal sections (III, IV бөлім).

Usage:
    python scripts/load_textbook_30.py --dry-run           # Parse only
    python scripts/load_textbook_30.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_30.py --update-content FILE # Update existing content
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 30  # Already exists in DB
TEXTBOOK_TITLE = "Қазақстан тарихы 11 сынып, 2-бөлім"
SUBJECT_CODE = "history_kz"
GRADE_LEVEL = 11
AUTHORS = "Кабульдинов З., Сандыбаева А.Д., Лебаев Ф.Р."
PUBLISHER = "Атамұра"
YEAR = 2020
ISBN = "978-601-331-714-4"
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "30" / "textbook_30.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "30"

# Chapter mapping: § number ranges → chapter info
# Continues from Part 1 (textbook 29 had chapters 1-5, §1-22)
CHAPTERS = [
    {
        "number": 6,
        "title": "Алтыншы тарау. ҚАЗАҚСТАНДАҒЫ ӘЛЕУМЕТТІК-САЯСИ ОЙДЫҢ ЭВОЛЮЦИЯСЫ",
        "para_from": 23,
        "para_to": 30,
    },
    {
        "number": 7,
        "title": "Жетінші тарау. «АЛАШ» — ҚОҒАМДЫҚ ОЙ ЖӘНЕ ҰЛТТЫҚ ИДЕЯ",
        "para_from": 31,
        "para_to": 34,
    },
    {
        "number": 8,
        "title": "Сегізінші тарау. «МӘҢГІЛІК ЕЛ» ЖАЛПЫҰЛТТЫҚ ИДЕЯСЫ — XXI ҒАСЫРДАҒЫ ҚАЗАҚСТАН ҚОҒАМЫН БІРІКТІРУШІ НЕГІЗ",
        "para_from": 35,
        "para_to": 38,
    },
    {
        "number": 9,
        "title": "Тоғызыншы тарау. ОРТАҒАСЫРЛЫҚ ҚАЗАҚСТАННЫҢ ҒЫЛЫМИ МҰРАСЫ",
        "para_from": 39,
        "para_to": 40,
    },
    {
        "number": 10,
        "title": "Оныншы тарау. XVIII-XX ҒАСЫРЛАРДАҒЫ ҚАЗАҚСТАНДАҒЫ БІЛІМ МЕН ҒЫЛЫМНЫҢ ДАМУЫ",
        "para_from": 41,
        "para_to": 48,
    },
    {
        "number": 11,
        "title": "Он бірінші тарау. БҮГІНГІ ТАҢДАҒЫ ҚАЗАҚСТАНДАҒЫ БІЛІМ ЖӘНЕ ҒЫЛЫМ ЖҮЙЕСІ",
        "para_from": 49,
        "para_to": 50,
    },
]
...
INTERNAL_HEADINGS = {
    ...
    "Шығармашылық тапсырма.", "Шығармашылық тапсырма",
    "Ойлан", "Ойла",
    ...
    "Анықта",
    "Ойталқы",
    "Зерттеу жүрrіз", "Зерттеу жүргіз",
    "Мәтінмен жүмыс.", "Мәтінмен жұмыс",
    # Person names / document titles inside paragraphs
    "Қожа Ахмет Ясауи",
    "ДАНАЛЫҚ КІТАБЫ",
    "Мүрат Мецкерлы",
    "Абайдыц сегізінші қарасезі", "Абайдың сегізінші қарасөзі",
    "«Қазақтың» жабылуы редакциялық ескертпе",
    "Сәкен Сейфуллинніц",
    "МЕН САПАСЫН АРТТЫРЫП",
    # Section-level headings (not paragraph boundaries)
    "қоғАмдық-САЯСИ ОЙДЫҢ ДАМУЫ",
    "ҚОҒАМДЫҚ-САЯСИ ОЙДЫҢ ДАМУЫ",
    "ҚАЗАҚСТАНДАҒЫ ӘЛЕУМЕТТІК-САЯСИ",
    "ОРТАҒАСЫРЛЫҚ ҚАЗАҚСТАННЫҢ ҒЫЛЫМИ М¥РАСЫ",
    "ОРТАҒАСЫРЛЫҚ ҚАЗАҚСТАННЫҢ ҒЫЛЫМИ МҰРАСЫ",
    "БүГІНГІ ТАҢДАҒЫ ҚАЗАҚСТАНДАҒЫ БІЛІМ ЖӘНЕ ҒЫЛЫМ ЖҮЙЕСІ",
    "БҮГІНГІ ТАҢДАҒЫ ҚАЗАҚСТАНДАҒЫ БІЛІМ ЖӘНЕ ҒЫЛЫМ ЖҮЙЕСІ",
    "БІЛІМ МЕН ҒЫЛЫМНЫҢ ДАМУЫ",
    # Chapter headings (mapped by § ranges, not headings)
    "Алтыншы тарау",
    "Жетінші тарау",
    "Сегізінші тарау",
    "Тоғызыншы тарау",
...
STOP_HEADINGS = {
    "Үшінші белім бойынша тест тапсырмалары",
    "Үшінші бөлім бойынша тест тапсырмалары",
    "Үшінші бөлімге қорытынды",
    "Тертінші белім бойынша тест тапсырмалары",
    "Төртінші бөлім бойынша тест тапсырмалары",
    "Тертінші бөлімге қорытынды",
    "Төртінші бөлімге қорытынды",
...
class ParsedParagraph:
    title: str
    number: int      # First § number (e.g., 23 for "§23-24")
    raw_number: str   # Original string (e.g., "23-24")
...
# Paragraph heading: "## §23-24. Title", "## §49-50. Title"
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        f"-- Қазақстан тарихы 11 сынып, 2-бөлім Import",
        f"-- Generated by load_textbook_30.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        "-- Қазақстан тарихы 11, 2-бөлім Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Қазақстан тарихы 11 (2-бөлім) textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Қазақстан тарихы 11 сынып, 2-бөлім — Textbook Import")
    print("=" * 70)
