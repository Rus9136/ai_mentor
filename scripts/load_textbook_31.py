#!/usr/bin/env python3
"""
Load 'Қазақстан тарихы 6-сынып' (Kazakh) into the AI Mentor database.
...
7 chapters (бөлім), 60 paragraph sections (§1 through §60),
compound numbering: §3-4, §20-21, §31-32, §48-49.

Usage:
    python scripts/load_textbook_31.py --dry-run           # Parse only
    python scripts/load_textbook_31.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_31.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 31  # Already exists in DB
TEXTBOOK_TITLE = "Қазақстан тарихы 6-сынып"
SUBJECT_CODE = "history_kz"
GRADE_LEVEL = 6
AUTHORS = "Т.Омарбеков, Г.Хабижанова, Т.Қартаева, М.Ноғайбаева"
PUBLISHER = "Мектеп"
YEAR = 2018
ISBN = "978-601-07-0976-8"
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "31" / "textbook_31.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "31"
...
CHAPTERS = [
    {
        "number": 1,
        "title": "I бөлім. VI-IX ҒАСЫРЛАРДАҒЫ ҚАЗАҚСТАН",
        "para_from": 1,
        "para_to": 15,
    },
    {
        "number": 2,
        "title": "II бөлім. X ҒАСЫР - XIII ҒАСЫРДЫҢ БАСЫНДАҒЫ ҚАЗАҚСТАН",
        "para_from": 16,
        "para_to": 29,
    },
    {
        "number": 3,
        "title": "III бөлім. XIII-XV ҒАСЫРДЫҢ БІРІНШІ ЖАРТЫСЫНДАҒЫ ҚАЗАҚСТАН",
        "para_from": 30,
        "para_to": 46,
    },
    {
        "number": 4,
        "title": "IV бөлім. ҚАЗАҚ ХАЛҚЫНЫҢ ҚАЛЫПТАСУЫ",
        "para_from": 47,
        "para_to": 47,
    },
    {
        "number": 5,
        "title": "V бөлім. БІРТҰТАС ҚАЗАҚ МЕМЛЕКЕТІНІҢ ҚҰРЫЛУЫ",
        "para_from": 48,
        "para_to": 49,
    },
    {
        "number": 6,
        "title": "VI бөлім. ҚАЗАҚ ХАНДЫҒЫНЫҢ XVI-XVII ҒАСЫРЛАРДАҒЫ ДАМУЫ",
        "para_from": 50,
        "para_to": 57,
    },
    {
        "number": 7,
        "title": "VII бөлім. XVI-XVII ҒАСЫРЛАРДАҒЫ ЭКОНОМИКА МЕН МӘДЕНИЕТ",
        "para_from": 58,
        "para_to": 60,
    },
]
...
INTERNAL_HEADINGS = {
    # Lesson structure
    "Бұл сабақта:", "Бул сабақта:", "Бұл сабақта: <br>",
...
class ParsedParagraph:
    title: str
    number: int      # First § number (e.g., 3 for "§ 3-4")
    raw_number: str   # Original string (e.g., "3-4")
...
# Paragraph heading: "## § 1.", "## §19.", "## § 20-21.", or plain "§16."
...
RE_STOP = re.compile(
    r"^#{1,2}\s+(Хронологиялык\b|Терминдер\b|Пайдаланылған\b|МАЗМУНЫ\b)",
    re.IGNORECASE,
)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if m:
            raw_num = m.group(1)  # e.g., "3-4", "20-21", "16"
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Қазақстан тарихы 6-сынып Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_31.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Қазақстан тарихы 6-сынып Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Қазақстан тарихы 6-сынып textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Қазақстан тарихы 6-сынып — Textbook Import")
    print("=" * 70)
