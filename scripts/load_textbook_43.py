#!/usr/bin/env python3
"""
Load 'Spotlight for Kazakhstan Grade 5' (English) into the AI Mentor database.
...
Chapters = Modules (Starter + 9 Modules), Paragraphs = Sections within modules.

Usage:
    python scripts/load_textbook_43.py --dry-run           # Parse only
    python scripts/load_textbook_43.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_43.py --update-content FILE # Update content
"""
import re
import sys
import hashlib
...
MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "43" / "textbook_43.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "43"

# Module definitions: chapter_number → module title
MODULES = [
    {"number": 1, "title": "Starter"},
    {"number": 2, "title": "Module 1 — Home & Away"},
    {"number": 3, "title": "Module 2 — Living Things!"},
    {"number": 4, "title": "Module 3 — Values"},
    {"number": 5, "title": "Module 4 — World of Work"},
    {"number": 6, "title": "Module 5 — Creativity"},
    {"number": 7, "title": "Module 6 — Reading for Pleasure"},
    {"number": 8, "title": "Module 7 — Fantasy World!"},
    {"number": 9, "title": "Module 8 — Sports"},
    {"number": 10, "title": "Module 9 — Holidays"},
]
...
class ParsedParagraph:
    ...
    section_key: str   # Unique key for merging duplicates (e.g., "Use of English 1b")
...
# Matches: Reading 1a, Use of English 2b, Writing 4 g, etc.
...
# ACROSS CULTURES (no code, just keyword)
RE_ACROSS_CULTURES = re.compile(
    r"^##\s+ACROSS CULTURES\b",
    re.IGNORECASE,
)

# EDUTAINMENT (various OCR forms: EDUTAINMENT, EDUTAINMENT-2, EDUTAINMENTC7)
...
# Module 6 story sections: "6a The Fisherman and the Fish"
RE_STORY_SECTION = re.compile(
    r"^##\s+(\d+)([a-z])\s+(.+)",
)

# Intro Vocabulary (no code, just "Vocabulary")
RE_VOCABULARY_INTRO = re.compile(
    r"^##\s+Vocabulary\s*$",
    re.IGNORECASE,
)

# MODULE heading (MODULE, module, modue)
RE_MODULE_HEADING = re.compile(
    r"^#{1,2}\s+(?:MODULE|module|modue)\b",
    re.IGNORECASE,
)

# Heading that's just "Writing (a postcard)" etc. (sub-heading, not section boundary)
...
def clean_heading_for_matching(line: str) -> str:
    """Remove LaTeX formatting from heading for pattern matching."""
    # $\mathbf{4 g}$ → 4 g
...
def detect_module_number(heading_text: str) -> int | None:
    ...
    # Extract number from LaTeX expressions BEFORE stripping
    # Handles: MODULE $2 \longdiv{...}$, MODULE $4 \sqrt{...}$
    latex_number = None
    ...
    # "MODULE" alone → use latex_number if available, else Module 1
    if re.match(r"MODULE\s*$", clean, re.IGNORECASE):
        if latex_number:
            return latex_number
        return 1

    # "modue 6..." → Module 6
    if re.match(r"modue\s+6", clean, re.IGNORECASE):
        return 6

    # "module 3..." → Module 3
    if re.match(r"module\s+3", clean, re.IGNORECASE):
        return 3

    # MODULE with content
    if re.match(r"MODULE\s+", clean, re.IGNORECASE):
        m = re.search(r"MODULE\s+(\d+)", clean, re.IGNORECASE)
        if m:
            return int(m.group(1))
        if "Fantasy" in clean:
            return 7
        if "World of Work" in clean:
            return 4
        if "Living" in clean:
            return 2

    # Special module titles without "MODULE" keyword
    if clean == "Creativity":
        return 5
    if clean == "Sports":
        return 8
...
def detect_section(line: str, current_module_num: int) -> tuple[str, str | None] | None:
    ...
    If title is None, the section should be merged into an existing one (e.g., VALUES → Edutainment).
    """
    heading_match = re.match(r"^(#{1,2})\s+(.+)", line)
    if not heading_match:
        return None

    # Clean LaTeX formatting for matching
    clean_line = clean_heading_for_matching(line)

    # Writing sub-heading like "## Writing (a postcard)" — NOT a section boundary
    ...
    # EDUTAINMENT
    if RE_EDUTAINMENT.match(clean_line):
    ...
    # Story section (Module 6): "6a The Fisherman..."
    m = RE_STORY_SECTION.match(clean_line)
    if m and int(m.group(1)) == 6:
        letter = m.group(2)
        story_title = m.group(3).strip()
        key = f"Story 6{letter}"
        title = f"6{letter}. {fix_ocr(story_title)}"
        return (key, title)

    # Vocabulary intro (just "Vocabulary", no code)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    # State variables
    current_module = 0       # 0 = preamble, -1 = starter, 1-9 = module number
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if heading_match:
            ...
            # --- Starter detection ---
            if heading_text.strip().upper() == "STARTER" and current_module == 0:
                # First Starter heading — create Starter paragraph
                current_module = -1
                ch = chapters[1]
            ...
            if mod_num is not None:
                ...
                chapter_num = mod_num + 1
                in_module_intro = True
                current_para = None
                if chapter_num not in para_counter:
                    para_counter[chapter_num] = 1
                continue

            # --- "What's in this module?" ---
            if "What's in this module" in heading_text:
                # Module 9 has no explicit MODULE heading
                if current_module == 8:
                    current_module = 9
                    chapter_num = 10
                    current_para = None
            ...
            if current_module >= 1:
                ...
                if section_info:
                    ...
                    if key in section_map:
                        ...
                        pnum = para_counter.get(chapter_num, 1)
                        current_para = ParsedParagraph(
                            title=title,
                            number=pnum,
                            section_key=key,
                        ...
                        para_counter[chapter_num] = pnum + 1
                    continue

            # --- Starter content: add heading as content ---
            if current_module == -1 and current_para:
                current_para.content_lines.append(line)
                continue
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Spotlight for Kazakhstan Grade 5 — Textbook Import",
        f"-- Generated by load_textbook_43.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Spotlight Grade 5 Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Spotlight for Kazakhstan Grade 5 into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Spotlight for Kazakhstan Grade 5 — Textbook Import")
    print("=" * 70)
