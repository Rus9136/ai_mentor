#!/usr/bin/env python3
"""
Load 'Информатика 7-сынып (Атамура)' into the AI Mentor database.

Parses the Mathpix MMD file, creates chapter/paragraph records,
and creates ParagraphContent records for language='kk'.

This is an INFORMATICS textbook — no exercises (A/B/C), 5 бөлім, 23 paragraphs + Кіріспе.
Paragraphs use N.M. numbering (e.g. 1.1, 2.3, 5.4).
Chapters are determined by paragraph prefix from МАЗМҰНЫ (TOC).
Has "Кіріспе" (OCR'd as "Kipicne") before §1.1.

Usage:
    python scripts/load_textbook_59.py --dry-run           # Parse only
    python scripts/load_textbook_59.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_59.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 59  # Already exists in DB
TEXTBOOK_TITLE = "Информатика 7-сынып (Атамура)"
...
MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "59" / "textbook_59.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "59"

# Chapter mapping: paragraph number ranges → chapter info (from МАЗМҰНЫ)
CHAPTERS = [
    {"number": 1, "title": "I бөлім. Компьютерлік жад және ақпараттық өлшем бірлік",
     "para_from": 0, "para_to": 5},
    {"number": 2, "title": "II бөлім. Желі және қауіпсіздік",
     "para_from": 6, "para_to": 8},
    {"number": 3, "title": "III бөлім. Электрондық кесте арқылы есеп шығару",
     "para_from": 9, "para_to": 14},
    {"number": 4, "title": "IV бөлім. Python тілінде алгоритмдерді программалау",
     "para_from": 15, "para_to": 19},
    {"number": 5, "title": "V бөлім. Практикалық программалау",
     "para_from": 20, "para_to": 23},
]

# Sequential mapping: (chapter_prefix, sub_number) → global paragraph number
# Кіріспе → 0; 1.1→1, 1.2→2, ..., 1.5→5; 2.1→6, 2.2→7, 2.3→8; etc.
_PARA_OFFSET = {1: 0, 2: 5, 3: 8, 4: 14, 5: 19}
...
INTERNAL_HEADINGS = {
    # Lesson structure sections (appear in every paragraph)
    "Нені үйренесіңдер?",
    "Нені үйренесіндер?",  # OCR variant
    "Тірек сөздер",
    "Тірек сездер",  # OCR variant
    # Bloom taxonomy assessment sections
    "Білу және түсіну",
    "Білу жән түсіну",  # OCR variant
    "Қолдану",
    "Талдау",
    "Жинақтау",
    "Бағалау",
    ...
    "Орындау әрекеті:",
    "Орындау әрекеті",
    # Sub-topic headings within specific paragraphs
    "Компьютерлік желілер:",
    "Компьютерлер жұмысын желіде ұйымдастыру",
    "Компьютерлік желілер топологиясы",
    "Ақпаратты бір өлшем бірліктен басқа өлшем бірлікке ауыстыру",
    "Деректер форматы:",
    "Форматтау кезінде:",
    "Автотолтыру мүмкіндіктері:",
    "Ақпараттың өмірлік циклі",
    "Ақпараттық құбылыс",
    "Ақпаратты беру әдістері:",
    "Ақпарат феномені",
    "Архиваторлар",
    "Ақылды кесте",
    "Ұяшықтардағы ашылмалы тізім",
    "Алгоритм:",
    "Программалау бойынша қосымша тапсырмалар",
    "Квадрат теңдеу",
    "Квадрат теңдеуді шешу",
    "Файлдарға арналған есептер",
    "Файл туралы қысқаша мәлімет",
    "Файлды жазу",
    "Файлды оқу",
    "Келесі жолдарды оқу",
    "Программаны түзету ерекшеліктері:",
    "Программаның қолмен жасалатын трассировкасы:",
    "Программаны құжаттандыру",
    "Алгоритмнің жалпы сипаттамасы қасиет болып табылады.",
    "Алған нәтиженің экологиялық интерпретациясы",
    "Шартты белгілері:",
    "Тапсырма:",
    # Modeling sub-steps
    "1. Тапсырманың қойылуы",
    "2. Модельдеудің міндеттерін анықтау",
    "3. Модельді жобалау",
    "4. Модельдеуді зерттеу",
    "5. Нәтижелерді талдау",
    # "Көп білгім келеді" enrichment sub-headings
    "Excel файлдарын Word-қа түрлендіруді (конвертация) орындаймыз. 1-тәсіл: Қолмен көшіру",
    "«Құпиясез бойынша ену» программасы",
    "Қадамдарды ретімен орындау:",
    # Menu bar / code snippets (OCR from screenshots)
    "File Edit Format Run Options Window Help",
    "RESTART:",
    "RESTART: C:/1",
    # Window titles (OCR from screenshots)
    "*Untitled*",
    # Code lines
    'f. write ("hello")',
    "f. close ()",
    # Image captions (## format)
    "Сканирование с 360 Total Security",
    # Enrichment subsections in Chapter 5
    "Codecademy",
    "Code School",
    "Motion Math Games",
    "Mathletics",
...
class ParsedParagraph:
    title: str
    number: int       # Global sequential number (0 for Кіріспе)
...
# Paragraph heading: "## 1.1. Title", "## 3.2. Title", "## 1.2. <br> Title"
# Title must start with a letter (avoids false matches like "## 2.2.-кестенің жалғасы")
RE_PARAGRAPH = re.compile(
    r"^#{1,2}\s+(\d+)\.(\d+)\.\s*(?:<br>\s*)?([А-ЯӘӨҮІҚҢҒҺа-яәөүіқңғһA-Za-z].+)",
    re.IGNORECASE,
)

# Introduction heading (OCR variants of "Кіріспе")
RE_INTRO = re.compile(r"^#{1,2}\s+Kip[іi][cs][npт][еe]\s*$", re.IGNORECASE)

# Stop markers — everything after these is glossary/appendix
RE_STOP = re.compile(
    r"^#{1,2}\s+(ГЛОССАРИЙ\b|ПАЙДАЛАНҒАН\s+ӘДЕБИЕТТЕР\b|ИНТЕРНЕТ-РЕСУРСТАР\b)",
    re.IGNORECASE,
)

# Test section markers (end of each chapter) — stop accumulating paragraph content
RE_TEST_SECTION = re.compile(
    r"^#{1,2}\s+(?:\d+|[IVX]+)\s*б[еоөeЕОӨE][лЛ][іiІI]мге\s+арналған\s+тест",
    re.IGNORECASE,
)

# Chapter marker headings (just a number or Roman numeral)
...
RE_LATEX_SECTION = re.compile(r"^#{1,2}\s+\\section\*\{(.+?)\}")
...
def is_internal_heading(heading: str) -> bool:
    ...
    # Heading that starts with a digit (task numbers, image captions, dates)
    ...
    # LaTeX \section*{} — treat as internal content heading
    if RE_LATEX_SECTION.match("## " + clean) or "\\section" in clean:
        return True

    # N бит / N байт type content headings
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    in_abstract = False
    in_preamble = True  # Before Кіріспе
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Test section markers — stop accumulating to current paragraph
        ...
        # Detect Кіріспе heading (OCR'd as "Kipicne")
        ...
        # Chapter marker headings (bare numbers like "## 1", "## 11") — skip
        ...
        if heading_match:
            ...
            if is_internal_heading(heading_text):
                ...
                # Unknown heading — include as content (safer than splitting)
                if current_paragraph is not None:
                    current_paragraph.content_lines.append(line)
                continue
...
def md_lines_to_html(lines: list[str], textbook_id: int) -> str:
    ...
    for raw_line in content.split("\n"):
        ...
        if heading_match:
            ...
            heading_text = heading_match.group(2).strip()
            # Clean LaTeX \section*{} artifacts
            sec_match = RE_LATEX_SECTION.match(line)
            if sec_match:
                heading_text = sec_match.group(1)
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        f"-- Информатика 7-сынып (Атамура) Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_59.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        "-- Информатика 7-сынып (Атамура) Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 7-сынып (Атамура) textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 7-сынып (Атамура) — Textbook Import")
    print("=" * 70)
