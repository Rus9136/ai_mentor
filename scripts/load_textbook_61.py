#!/usr/bin/env python3
"""
Load 'Информатика 8-сынып (Атамура)' into the AI Mentor database.

Parses the Mathpix MMD file, creates chapter/paragraph records,
and creates ParagraphContent records for language='kk'.

This is an INFORMATICS textbook — no exercises (A/B/C), 5 бөлім, 19 paragraphs + Кіріспе.
Paragraphs use N.M. numbering (e.g. 1.1, 3.4, 5.4).
Chapters are determined by paragraph prefix from МАЗМҰНЫ (TOC).
Has "Кіріспе" (OCR'd as "Kipicne") before §1.1.
Special: Paragraph 2.1 has NO explicit heading — created from chapter II marker.

Usage:
    python scripts/load_textbook_61.py --dry-run           # Parse only
    python scripts/load_textbook_61.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_61.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 61  # Already exists in DB
TEXTBOOK_TITLE = "Информатика 8-сынып (Атамура)"
...
MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "61" / "textbook_61.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "61"

# Chapter mapping: paragraph number ranges → chapter info (from МАЗМҰНЫ)
# Кіріспе→0; 1.1→1, 1.2→2, 1.3→3; 2.1→4, 2.2→5; 3.1→6..3.4→9; 4.1→10..4.6→15; 5.1→16..5.4→19
CHAPTERS = [
    {"number": 1, "title": "I бөлім. Компьютер мен желілердің техникалық сипаттамалары",
     "para_from": 0, "para_to": 3},
    {"number": 2, "title": "II бөлім. Денсаулық және қауіпсіздік",
     "para_from": 4, "para_to": 5},
    {"number": 3, "title": "III бөлім. Электрондық кестелердегі ақпаратты өңдеу",
     "para_from": 6, "para_to": 9},
    {"number": 4, "title": "IV бөлім. Python тілінде алгоритмдерді программалау",
     "para_from": 10, "para_to": 15},
    {"number": 5, "title": "V бөлім. Практикалық программалау",
     "para_from": 16, "para_to": 19},
]

# Sequential mapping: (chapter_prefix, sub_number) → global paragraph number
_PARA_OFFSET = {1: 0, 2: 3, 3: 5, 4: 9, 5: 15}
...
OCR_FIXES = {
    ...
    "）": ")",
    "？": "?",
    "елшеу": "өлшеу",  # OCR error in 1.1 title
}
...
class ParsedParagraph:
    ...
    raw_number: str   # Original string (e.g. "1.1", "2.1")
...
# Paragraph heading: "## 1.1. Title", "### 4.1. While циклі"
# Title must start with a letter (avoids false matches)
...
# Matches: "N белімге қосымша тапсырмалар", "N белім бойынша қорытынды тест", etc.
RE_TEST_SECTION = re.compile(
    r"^#{1,2}\s+.+(?:қосымша\s+тапсырмалар|қорытынды\s+тест)",
    re.IGNORECASE,
)

# Chapter marker headings (just a number or Roman numeral, e.g. "## IV", "## 1")
RE_CHAPTER_MARKER = re.compile(
    r"^#{1,2}\s+(?:[IVX]+\s*$|\d{1,2}\s*$)", re.IGNORECASE
)

# Special: Chapter II marker — "## 1" (OCR-damaged, should be "## II")
# Paragraph 2.1 has no explicit heading; it starts after this marker
RE_CHAPTER_II_MARKER = re.compile(r"^#{1,2}\s+1\s*$")
...
def is_internal_heading(heading: str) -> bool:
    ...
    # Starts with image ref
    if clean.startswith("!["):
        return True

    # Starts with $ (math formula)
    if clean.startswith("$"):
        return True

    # Starts with a digit (task numbers, image captions)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Test section markers — stop accumulating paragraph content
        ...
        # Special case: Chapter II marker "## 1" (OCR-damaged)
        # Paragraph 2.1 has no explicit heading — create it here
        if RE_CHAPTER_II_MARKER.match(line) and in_test_section:
            in_test_section = False
            current_paragraph = ParsedParagraph(
                title="Компьютерді пайдаланудың кері аспектілері",
                number=calc_global_number(2, 1),
                raw_number="2.1",
            )
            paragraphs.append(current_paragraph)
            continue
        ...
        # Chapter marker headings (bare numbers like "## IV", "## 11") — skip
        if RE_CHAPTER_MARKER.match(line):
            continue

        # All other headings — include as content
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        f"-- Информатика 8-сынып (Атамура) Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_61.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        "-- Информатика 8-сынып (Атамура) Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 8-сынып (Атамура) textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 8-сынып (Атамура) — Textbook Import")
    print("=" * 70)
