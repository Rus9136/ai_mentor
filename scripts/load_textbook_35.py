#!/usr/bin/env python3
"""
Load 'Информатика 7-сынып' into the AI Mentor database.
...
Some lesson headings lack ## prefix (1.4, 5.3, 5.5, 5.6-5.7).

Usage:
    python scripts/load_textbook_35.py --dry-run           # Parse only
    python scripts/load_textbook_35.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_35.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 35
TEXTBOOK_TITLE = "Информатика 7-сынып"
SUBJECT_CODE = "informatics"
GRADE_LEVEL = 7
AUTHORS = "Кадыркулов Р., Рыскулбекова А., Нурмуханбетова Г."
PUBLISHER = "Алматыкітап"
YEAR = 2023
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "35" / "textbook_35.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "35"

# Chapter mapping based on МАЗМУНЫ (TOC)
CHAPTERS = [
    {
        "number": 1,
        "title": "I бөлім. Компьютерлік жад және ақпараттың өлшем бірліктері",
        "paragraphs": [
            {"raw": "1.1", "title": "Ақпараттың өлшем бірліктері"},
            {"raw": "1.2", "title": "Компьютерлік жад"},
            {"raw": "1.3", "title": "Файлдардың форматтары"},
            {"raw": "1.4", "title": "Файлдардың мөлшері"},
        ],
    },
    {
        "number": 2,
        "title": "II бөлім. Желі және қауіпсіздік",
        "paragraphs": [
            {"raw": "2.1", "title": "Компьютерлік желілер мен олардың жіктелуі"},
            {"raw": "2.2", "title": "Антивирустық қауіпсіздік"},
            {"raw": "2.3", "title": "Пайдаланушы интерфейсі"},
            {"raw": "2.4", "title": "Практикалық жұмыс"},
        ],
    },
    {
        "number": 3,
        "title": "III бөлім. Электрондық кесте арқылы есеп шығару",
        "paragraphs": [
            {"raw": "3.1", "title": "Мәтіндік процессордағы кестелер"},
            {"raw": "3.2", "title": "Электрондық кестелер"},
            {"raw": "3.3", "title": "Электрондық кестенің элементтерін форматтау"},
            {"raw": "3.4", "title": "Деректер типтері"},
            {"raw": "3.5", "title": "Шартты форматтау"},
            {"raw": "3.6", "title": "Кестелік деректерді графикалық түрде ұсыну"},
            {"raw": "3.7", "title": "Процестерді электрондық кестеде модельдеу"},
        ],
    },
    {
        "number": 4,
        "title": "IV бөлім. Python тіліндегі алгоритмдерді программалау",
        "paragraphs": [
            {"raw": "4.1", "title": "Файлдармен жұмыс"},
            {"raw": "4.2", "title": "Файлдық функциялармен жұмыс"},
            {"raw": "4.3-4.4", "title": "Тармақталған алгоритмдерді программалау"},
            {"raw": "4.5", "title": "Кірістірілген шарттарды программалау"},
            {"raw": "4.6", "title": "Құрамды шарттарды программалау"},
            {"raw": "4.7", "title": "Практикалық жұмыс"},
            {"raw": "4.8", "title": "Таңдауды ұйымдастыру"},
            {"raw": "4.9-4.10", "title": "Практикалық жұмыс"},
        ],
    },
    {
        "number": 5,
        "title": "V бөлім. Практикалық программалау",
        "paragraphs": [
            {"raw": "5.1", "title": "Проблеманы қою"},
            {"raw": "5.2", "title": "Алгоритмді әзірлеу"},
            {"raw": "5.3", "title": "Алгоритмді программалау"},
            {"raw": "5.4", "title": "Есепті программалау. Практикалық жұмыс"},
            {"raw": "5.5", "title": "Программаны тестілеу"},
            {"raw": "5.6-5.7", "title": "Практикалық жұмыс"},
        ],
    },
]
...
TOTAL_EXPECTED_PARAGRAPHS = _seq  # 29

# When MMD uses compound heading format "## 4.3-4.4 Title"
# Map first-number-only to compound: these have ## with dash in heading
COMPOUND_HEADING_MAP = {
    "4.3-4.4": "4.3-4.4",
    "4.9-4.10": "4.9-4.10",
    "5.6-5.7": "5.6-5.7",
}
...
class ParsedParagraph:
    ...
    raw_number: str      # Original string (e.g., "1.1", "4.3-4.4")
...
# Standard paragraph heading: "## 1.1 Title" or "## 4.3-4.4 Title"
RE_PARA_HEADING = re.compile(
    r"^#{1,3}\s+(\d+\.\d+(?:-\d+\.\d+)?)\s+(.*)", re.IGNORECASE
)

# Bare paragraph heading (no ##): "1.4 Файлдардың мөлшері", "5.3 Алгоритмді..."
# Must be at line start, N.N followed by space and Kazakh/Cyrillic title
RE_PARA_BARE = re.compile(
    r"^(\d+\.\d+(?:-\d+\.\d+)?)\s+([А-ЯӘҒҚҢӨҰҮҺІа-яәғқңөұүһі].+)"
)

# Chapter heading: "## II бөлім", "І бөлім" etc.
RE_CHAPTER = re.compile(
    r"^(?:#{1,2}\s+)?([IVІ]+)\s+[Бб][өо]лі[мн]", re.IGNORECASE
)

# Section summary heading
RE_SECTION_SUMMARY = re.compile(
    r"^#\s+Бөлімнің қорытындысы", re.IGNORECASE
)

# Test section heading
RE_TEST_SECTION = re.compile(
    r"^#{1,2}\s+Өзіңді тексер!", re.IGNORECASE
)

# Stop markers
RE_STOP = re.compile(
    r"^#{1,2}\s+(Глоссарий\b|Пайдаланылған\b|МАЗМУНЫ\b)",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    internal_exact = {
        ...
        "Ойлан", "Д Жаңа білім", "Жаңа білім", "1 Жаңа білім",
        "Д) Жаңа білім",
        ...
        "Шартты белгілер:",
        "Құрметті оқушы!",
        "Р.А. Қадырқұлов, Ә.Д. Рысқұлбекова, Г.К. Нұрмұханбетова",
        "ИНФОРМАТИКА",
        "Жалпы білім беретін мектептің 7-сынып оқушыларына арналған оқулық",
        ...
        # 1.3 internal
        "Мектеп.doc",
        "Файлмен жұмыс істеу кезінде есіңде сақта!",
        # 1.4 internal
        "Мысалы: WinRAR, 7-Zір немесе WinZip.",
        "Файлдарды архивтеу",
        ...
        "Ведомстволық желі",
        "Мемлекеттік желі",
        "Деректерді тасымалдаужылдамдығына қарай бөлінуі:",
        "Деректерді тарату ортасьна байланысты бөлінуі:",
        "Кабельдік арналар",
        "Сымсыз желі",
        "Сымсыз байланыс арналарыныц түрлері:",
        "Желілер топологиясы",
        # 2.2 internal
        ...
        # 3.1 internal
        "Мысалы:",
        "Мәтіндік редакторда кесте құру",
        "Кестеге жол мен баған қосу",
        "Кесте ұяшықтарын бірнеше бөлікке бөлу мен біріктіру",
        "WORD мәтіндік редакторында кесте жасау үшін орындалатын әрекеттер:",
        "Адамзаттың өмірін күрт өзгерткен техникалық жетістіктер",
        # 3.2 internal
        "Excel программасы",
        "Excel-ді қолдану аумағы",
        "MS Excel программасын іске қосу жолдары",
        "Excel терезесінің бөліктері",
        "Книга1－Microsoft Excel",
        "Ұяшықты толтыру ережелері",
        "Сан", "Мәтін",
        "Үяшықтардың адрестері",
        # 3.3 internal
        ...
        "Ай, апта, күн тізімін енгізу тәсілі",
        "Уяшықтарды автоматты қосу",
        "Уяшықтарға формуланы автоорналастыру",
        "Жыл қайыру кестесін жасау",
        # 3.5 internal
        "Шартты форматтау түрлері",
        "«Сынып үлгерімі» жобасын жасау",
        "Жеңіл атлетикадан жарыс",
        # 3.6 internal
        "Диаграммалар",
        "Мысалдағы тапсырманы орындау үшін:",
        "Тапсырмаға берілген әрекеттерді орындаймыз.",
        "«Тоқсандық бағалар» жобасы",
        # 3.7 internal
        "Компьютерде модельдеу теориясы",
        "Компьютерде модельдеудің кезеңдері",
        "Қарапайым есептерді электрондық кестелерде модельдеу",
        "Ұяшықтың адресі",
        "Компьютерде модель құрамыз!",
        # Section summary internal
        "Тапсырманы орындаудың алгоритмі：",
        "Тапсырманы орындаудың алгоритмі:",
        # 4.1 internal
        ...
        "if шарт: <br> 1-командалар блогы 2-командалар блогы",
        "if шарт :",
        ...
        "©- Бағалау",
        # 5.4 internal
        "Тармақталу алгоритмдеріне арналған есептердің блок-схемасын және программасын құру",
        "Сызықтық алгоритмдерге арналған есептердің блок-схемасы мен программасын құрастыру",
        # 5.5 internal
        "Тестілеу процесінің кезеңдері",
        "Бағытқа ауысу",
        # 5.6-5.7 internal
        "Сызықтық алгоритмдерге үлгілік тестілер құрастыру мен программалау",
        "Тармақталу алгоритмдеріне үлгілік тестілер құрастыру мен программалау",
        # Bibliography
        "І. Оқулық әзірлеу кезінде пайдаланылған монографиялар",
        "II. Оқулық әзірлеу кезінде пайдаланылған сайттар тізімі",
        "III. Оқулықтағы тест жинақтары алынған электрондық форматтағы ресурстар",
        "Пайдаланылған әдебиеттер",
        # Schema diagrams
        "2-схема. Қысқа жазылу құрылымы",
        "3-схема. Толық жазылу құрылымы",
    ...
    internal_prefixes = [
        ...
        "9-тапсырма", "10-тапсырма", "11-тапсырма", "12-тапсырма",
        "13-тапсырма",
        "Ойлануға берілген сұрақтар",
        "І БӨЛІМ", "ІІ БӨЛІМ", "III БӨЛІМ", "IV БӨЛІМ", "V БӨЛІМ",
        "І бөлім", "II бөлім", "III бөлім", "IV бөлім", "V бөлім",
        "I БӨЛІМ", "I бөлім",
        "Кадиркулов", "Кадырқұлов",
        "Жалпы білім беретін",
        "1-есеп", "2-есеп", "3-есеп", "4-есеп", "5-есеп",
        "6-есеп", "7-есеп", "8-есеп", "9-есеп", "10-есеп",
        "11-есеп", "12-есеп",
        "Пуск ⟶",
        "1-әдіс", "2-әдіс",
        "3-кесте",
        "＊",
        "«Қазақстандық суретшілер»",
        "Гистограмманы оқимыз",
        "Графика құру",
        "Сұранысқа ие мамандықтар",
        "Қазақстан халқыныц",
        "Даладағы қуғын",
        "Өзіңді тексер",
        "Бөлімнің қорытындысы",
    ]
    for prefix in internal_prefixes:
        if clean.startswith(prefix):
            return True

    # Contains section/method markers
    if re.match(r"^\d+-схема\.", clean):
        return True
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Test sections: "Өзіңді тексер!" — skip until next paragraph
        if RE_TEST_SECTION.match(line):
            in_test_section = True
            in_summary_section = False
            continue

        # Section summary: "# Бөлімнің қорытындысы" — skip until next paragraph
        if RE_SECTION_SUMMARY.match(line):
        ...
        # Try to match paragraph heading with ## prefix
        ...
        if m_heading:
            ...
            if info and raw_num not in seen_raw_numbers:
                ...
                # Use TOC title if OCR title is mangled
            ...
            if current_paragraph is not None and not in_test_section and not in_summary_section:
                current_paragraph.content_lines.append(line)
            continue

        # Try bare paragraph heading (no ##): "1.4 Title", "5.3 Title"
        m_bare = RE_PARA_BARE.match(line)
        if m_bare:
            raw_num = m_bare.group(1)
        ...
        # If in test or summary section, skip content
        if in_test_section or in_summary_section:
            continue
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Информатика 7-сынып Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_35.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Информатика 7-сынып Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 7-сынып textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 7-сынып — Textbook Import")
    print("=" * 70)
