#!/usr/bin/env python3
"""
Load 'Информатика 8-сынып' into the AI Mentor database.
...
Heading formats vary: ## N.N, ### N.N, or bare N.N without any prefix.
Paragraph 3.1 has a split heading (## 3.1 + ## Title on separate lines).

Usage:
    python scripts/load_textbook_36.py --dry-run           # Parse only
    python scripts/load_textbook_36.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_36.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 36
TEXTBOOK_TITLE = "Информатика 8-сынып"
SUBJECT_CODE = "informatics"
GRADE_LEVEL = 8
AUTHORS = "Қадырқұлов Р.А., Рысқұлбекова Ә.Д., Нұрмұханбетова Г.К."
PUBLISHER = "Алматыкітап"
YEAR = 2021
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "36" / "textbook_36.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "36"

# Chapter mapping based on МАЗМУНЫ (TOC)
CHAPTERS = [
    {
        "number": 1,
        "title": "I бөлім. Компьютер мен желілердің техникалық сипаттамалары",
        "paragraphs": [
            {"raw": "1.1", "title": "Ақпаратты өлшеу. Алфавиттік әдіс"},
            {"raw": "1.2", "title": "Ақпаратты өлшеу. Ықтималдық әдісі"},
            {"raw": "1.3", "title": "Компьютердің ішкі құрылымы мен қосымша құрылғылары"},
            {"raw": "1.4", "title": "Процессор мен оның сипаттамалары"},
            {"raw": "1.5", "title": "Компьютерлік желілер"},
        ],
    },
    {
        "number": 2,
        "title": "II бөлім. Денсаулық және қауіпсіздік",
        "paragraphs": [
            {"raw": "2.1", "title": "Компьютерді пайдаланудың кері аспектілері"},
            {"raw": "2.2", "title": "Желідегі қауіпсіздік"},
        ],
    },
    {
        "number": 3,
        "title": "III бөлім. Ақпаратты электронды кестелерде өңдеу",
        "paragraphs": [
            {"raw": "3.1", "title": "Статистикалық деректер. Абсолютті және салыстырмалы сілтемелерді қолдану"},
            {"raw": "3.2", "title": "Абсолютті және салыстырмалы сілтемелерді практикалық тұрғыдан қолдану"},
            {"raw": "3.3", "title": "Кірістірілген функциялар"},
            {"raw": "3.4", "title": "Кірістірілген функцияларды практикалық тұрғыдан қолдану"},
            {"raw": "3.5", "title": "Қолжетімді ақпараттың негізінде деректерді талдау"},
            {"raw": "3.6-3.7", "title": "Қолданбалы есептерді шешу"},
            {"raw": "3.8", "title": "Excel программасында жобалық жұмыстар орындау"},
        ],
    },
    {
        "number": 4,
        "title": "IV бөлім. Python тілінде алгоритмдерді программалау",
        "paragraphs": [
            {"raw": "4.1", "title": "For циклі"},
            {"raw": "4.2", "title": "Кірістірілген циклдерді ұйымдастыру"},
            {"raw": "4.3", "title": "While циклі"},
            {"raw": "4.4", "title": "Continue циклін басқару"},
            {"raw": "4.5", "title": "Break циклін басқару"},
            {"raw": "4.6", "title": "Else циклін басқару"},
            {"raw": "4.7", "title": "Алгоритмнің трассировкасы"},
            {"raw": "4.8-4.9", "title": "Программалауды үйренейік. Практикалық тапсырмалар"},
            {"raw": "4.10", "title": "Программалау бойынша жоба жұмыстарын орындау"},
        ],
    },
    {
        "number": 5,
        ...
        "paragraphs": [
            {"raw": "5.1", "title": "Мәселе қою (қалыптастыру)"},
            {"raw": "5.2", "title": "Алгоритмді әзірлеу"},
            {"raw": "5.3-5.4", "title": "Алгоритмді программалау. Практикалық жұмыс"},
            {"raw": "5.5-5.6", "title": "Программаны тестілеу"},
            {"raw": "5.7", "title": "Олимпиадаға дайындық. Практикалық тапсырмалар"},
        ],
    },
]
...
class ParsedParagraph:
    ...
    raw_number: str      # Original string (e.g., "1.1", "3.6-3.7")
...
# Standard paragraph heading: "## 1.1 Title", "### 4.1 Title", "## 2.2. Title"
RE_PARA_HEADING = re.compile(
    r"^#{1,3}\s+(\d+\.\d+(?:-\d+\.\d+)?)\.?\s+(.*)", re.IGNORECASE
)

# Split heading: "## 3.1" alone on a line (title on next heading line)
RE_PARA_NUMBER_ONLY = re.compile(r"^#{1,3}\s+(\d+\.\d+(?:-\d+\.\d+)?)\s*$")

# Bare paragraph heading (no ##): "1.3 Компьютердің...", "3.8 Excel..."
RE_PARA_BARE = re.compile(
    r"^(\d+\.\d+(?:-\d+\.\d+)?)\.?\s+([А-ЯӘҒҚҢӨҰҮҺІа-яәғқңөұүһіA-Z].+)"
)

# Chapter heading: "## IV бөлім", "I бөлім" etc.
RE_CHAPTER = re.compile(
    r"^(?:#{1,2}\s+)?([IVІ]+)\s+[Бб][өо]лі[мн]", re.IGNORECASE
)

# Stop markers — dictionary, glossary, TOC, references
RE_STOP = re.compile(
    r"^#{1,2}\s+(Сөздік\b|Глоссарий\b|ГЛОССАРИЙ\b|Пайдаланылған\b|МАЗМУНЫ\b)",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    internal_exact = {
        ...
        "I Жаңа білім",
        "Талдау", "Жинақтау", "Бағалау", "© Бағалау", "©- Бағалау",
        "Тапсырма", "Тапсырма:",
        "Үй тапсырмасы", "Пе Уй тапсырмасы", "Па Тй тапсырмасы",
        "Пй Тй тапсырмасы",
        "Практикалық жұмыс", "- Практикалық жұмыс",
        "Шығармашылық тапсырмалар",
        "Шығармашылық-практикалық тапсырмалар",
        "Пікірталас",
        # Preamble
        "Шартты белгілер:",
        "Құрметті оқушы!",
        ...
        # 1.2 internal
        "Ақпаратты өлшеу",
        "Ақпарат көлемін анықтаудың ықтималдық әдісі",
        # 1.3 internal
        "Компьютердің ішкі құрылымы",
        "Аналық тақша",
        "Компьютердің слот кецейтілімдері:",
        # 1.4 internal
        "Процессордың негізгі параметрлері",
        "Процессордыц құрылысы",
        "Процессордың ядролар саны",
        # 1.5 internal
        "Компьютерлік желіге пайдаланылатын құралдар",
        "Бұлттық технология. Бұлттық есептеулер",
        "Желінің өткізу қабілеті",
        # 2.1 internal
        "Электрмагнитті сәулелердің адам ағзасына әсері",
        "Компьютер экранының электрмагниттік сәулеленуінің әсері",
        "Компьютерді пайдаланудың негізгі ережелері",
        # 2.2 internal
        "Ақпараттық қауіпсіздік",
        "Интернет желісіндегі алаяқтық",
        # 3.1 internal
        "Статистикалық деректер. Абсолютті және салыстырмалы сілтемелерді қолдану",
        ...
        "Мысалы:",
        # 3.3 internal
        "Функциялар. «Функция» ұғымына анықтама",
        ...
        # 3.5 internal
        "Excel-дегі тренд сызықтарының түрлері",
        "R-квадрат мәні",
        "Функция графигі",
        "Бізді коршаган диаграммалар",
        # 3.6-3.7 internal
        "Математикалық есептерді шешуде электрондық кестені қолдану",
        ...
        "Excel программасын тұрмыста колданудын тиімді жолдары",
        "Excel программасында тест жасау",
        # 3.8 internal
        "Мақтау қағазы",
        # 4.1 internal
        "Шексіз цикл",
        ...
        # 4.10 internal
        "Python программалау тілінде программалаудын манызы",
        "Программалау көмегімен колданбаль есептерді шешу",
        "Геомериялью есептерді программалау",
        "Жобаны қорғау кезінде:",
        "Жобаны бағалау барысында:",
        # 5.1 internal
        "«Ағашты тец бөліктерге бөлу» жобасы",
        "Есептің алгоритмі:",
        # 5.2 internal
        "Жиі қолданылатын дайын алгоритмдерді білу неге маңызды?",
        # 5.3-5.4 internal
        # 5.5-5.6 internal
    ...
    internal_prefixes = [
        ...
        "1-практикалық", "2-практикалық", "3-практикалық",
        "1-жоба", "2-жоба", "3-жоба", "4-жоба", "5-жоба",
        "6-жоба", "7-жоба", "8-жоба",
        "1-үлгі", "2-үлгі", "3-үлгі", "3-улгі",
    ...
    # Contains cycle control heading
    if "циклді басқару" in clean.lower():
        return True
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    in_abstract = False
    stopped = False
    # For handling split headings like "## 3.1" + "## Title"
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Stop markers (Сөздік, Глоссарий, etc.)
        ...
        # Handle pending split heading: "## 3.1" was on previous line,
        # now we expect "## Title" on this line
        if pending_para_num is not None:
            ...
            if heading_match:
                ...
                # Empty line after number-only heading — use known title
                title = ""

            info = resolve_paragraph_number(pending_para_num)
            if info and pending_para_num not in seen_raw_numbers:
                seen_raw_numbers.add(pending_para_num)
                known_title = _get_known_title(pending_para_num)
                current_paragraph = ParsedParagraph(
                    title=known_title or title,
                    number=info[1],
                    raw_number=pending_para_num,
                )
        ...
        # Try bare paragraph heading (no ##): "1.3 Title", "3.5 Title"
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Информатика 8-сынып Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_36.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Информатика 8-сынып Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 8-сынып textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 8-сынып — Textbook Import")
    print("=" * 70)
