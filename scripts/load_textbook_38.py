#!/usr/bin/env python3
"""
Load 'Информатика 10-сынып' into the AI Mentor database.
...
Some headings use ### instead of ## (3.3, 4.5, 5.4).
Test and project sections are excluded from paragraph content.

Usage:
    python scripts/load_textbook_38.py --dry-run           # Parse only
    python scripts/load_textbook_38.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_38.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 38  # Already exists in DB
TEXTBOOK_TITLE = "Информатика 10-сынып"
SUBJECT_CODE = "informatics"
GRADE_LEVEL = 10
AUTHORS = "Исабаева Д.Н., Абдулкаримова Г., Шекербекова Ш., Рахимжанова Л., Курмангалиева Н., Бекмолдаева А."
PUBLISHER = "Атамура"
YEAR = 2019
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "38" / "textbook_38.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "38"

# Chapter mapping based on МАЗМҰНЫ (TOC)
CHAPTERS = [
    {
        "number": 1,
        "title": "I бөлім. АҚПАРАТТЫҚ ҚАУІПСІЗДІК",
        "paragraphs": [
            {"raw": "1.1", "title": "Ақпаратты қорғау"},
            {"raw": "1.2", "title": "Ақпаратты қорғау әдістері. Резервтік көшіру және шифрлеу"},
            {"raw": "1.3", "title": "Сәйкестендіру және аутентификациялау"},
        ],
    },
    {
        "number": 2,
        "title": "II бөлім. ВИДЕОКОНТЕНТ ҚҰРУ",
        "paragraphs": [
            {"raw": "2.1", "title": "Видеомен жұмыс істеуге арналған программалар мен негізгі ұғымдар"},
            {"raw": "2.2", "title": "Видеоны түсіру және видеомонтаждың негізгі қағидалары"},
            {"raw": "2.3", "title": "Видеожазбаны өңдеу"},
        ],
    },
    {
        "number": 3,
        "title": "III бөлім. ДИЗАЙН ТЕОРИЯСЫ",
        "paragraphs": [
            {"raw": "3.1", "title": "Біздің өміріміздегі дизайн"},
            {"raw": "3.2", "title": "«Жақсы дизайн» принциптері"},
            {"raw": "3.3", "title": "Web-бетке арналған графика"},
            {"raw": "3.4", "title": "Сайт дизайнын әзірлеу"},
        ],
    },
    {
        "number": 4,
        "title": "IV бөлім. WEB-ЖОБАЛАУ",
        "paragraphs": [
            {"raw": "4.1", "title": "Сайттың картасы"},
            {"raw": "4.2", "title": "Сайттың негізгі беті"},
            {"raw": "4.3", "title": "Контент"},
            {"raw": "4.4", "title": "Тізім мен кестені қосу"},
            {"raw": "4.5", "title": "Web-парақтағы мультимедиа"},
            {"raw": "4.6", "title": "Сайтты жариялау"},
            {"raw": "4.7", "title": "Сайтты насихаттау"},
        ],
    },
    {
        "number": 5,
        "title": "V бөлім. ЖАСАНДЫ ИНТЕЛЛЕКТ ЖӘНЕ BLOCKCHAIN ТЕХНОЛОГИЯСЫ",
        "paragraphs": [
            {"raw": "5.1", "title": "Жасанды интеллект — заманауи ақпараттық технологияның негізі"},
            {"raw": "5.2", "title": "Машиналық оқыту принциптері"},
            {"raw": "5.3", "title": "Нейрондық желіні ұйымдастыру принциптері"},
            {"raw": "5.4", "title": "Blockchain (блокчейн) технологиясы"},
        ],
    },
]
...
class ParsedParagraph:
    ...
    raw_number: str      # Original string (e.g., "1.1", "3.3")
...
# Standard paragraph heading: "## 1.1. Title", "## 2.3 Title", "### 3.3 Title"
RE_PARA_STANDARD = re.compile(
    r"^#{1,3}\s+(\d+\.\d+)\.?\s+(.+)", re.IGNORECASE
)

# Chapter heading: "## І БӨЛІМ", "## IV БӨЛІМ", "## ІІІ БӨЛІМ. ТЕСТ"
RE_CHAPTER = re.compile(
    r"^#{1,2}\s+(?:I{1,3}|IV|V|ІІ?І?|ІV)\s+Б[ΘОоeө][лl][IіiIM]+", re.IGNORECASE
)

# Test and project sections — stop accumulating content
RE_SECTION_STOP = re.compile(
    r"^#{0,2}\s*(?:I{1,4}|IV|V|ІІ?І?|ІV|II?)\.?\s+"
    r"(?:БӨЛІМ\.?\s+ТЕСТ\b|ТО[қк]САН\s+БОЙЫНША\s+ЖОБАЛЫҚ)",
    re.IGNORECASE,
)

# Global stop markers — end of textbook content
RE_STOP = re.compile(
    r"^#{1,2}\s+(глоссАРИЙ\b|ГЛОССАРИЙ\b|ПАЙДАЛАНЫЛҒАН\b|МАЗМҰНЫ\b)",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    # Heading starts with image, formula
    if clean.startswith("![") or clean.startswith("$"):
        return True

    # Starts with digit (numbered items, figure refs)
    ...
    # Figure references
    if "сурет" in clean.lower() or "cypet" in clean.lower() or "cypem" in clean.lower():
        return True

    # Starts with → (instruction steps)
    if clean.startswith("→"):
        return True

    # Known internal headings — exact or prefix match
    internal_exact = {
        # Lesson structure / info blocks
        "Есте сақтаңдар!", "Есте сактандар!", "Есте сақтацдар!",
        "Eсте сақтацдар!", "Есте сақтаңдар:", "Маңызды!",
        # Textbook preamble
        "ИНФОРМАТИКА", "Сәттілік тілейміз!",
        "Қымбатты жоғары сынып оқушылары!",
        # Section 1.1 internal
        "Ақпараттық қауіпсіздікті құқықтық қамтамасыз ету",
        # Section 1.2 internal
        "Деректерді шифрлеу",
        # Section 1.3 internal
        "Аутентификациялау әдістері:",
        "Тіркелу жазбасын құру.",
        "Тігінен:", "Зерттеу жүргізіңдер.",
        # Section 2.1 internal
        "Видеоконтент және оның түрлері",
        "Видеомен жұмыс істеуге арналған программалар",
        # Section 2.2 internal
        "Видеомонтаж барысында келесі терминдер кездеседі:",
        "Видеомонтаждың негізгі қағидалары",
        "Видеоны түсіру және монтаждау ережелері",
        "Буктрейлер дайындау",
        # Section 2.3 internal
        "Видеофайлдарды түсіру және монтаждау",
        # Section 3.2 internal
        "Бетті қарау схемасы",
        "Айтрекинг - адамдардың сайтты қалай қарайтынын (оқитынын) зерттейді.",
        # Section 3.4 internal
        "Web-6еттің негізгі элементтері",
        "Web-6eт макеттері",
        # Practical works (in III БӨЛІМ)
        "Логотиптердің негізгі түрлері",
        "Баннердің негізгі фоны",
        "Текстура құру",
        "Баннер. «Әрекет жасауға шақыру» батырмасы",
        "«Отбрасываемая тень» терезесі",
        # Section 4.2 internal
        "Сайттың негізгі беті",
        # Section 4.3 internal
        "Мәтіндік контент", "Блог",
        "Графикалық контент", "Видео", "Инфографика",
        "Микросервистер", "Сауалнамалар",
        # Section 4.4 internal
        "WIX-те тізім құру",
        "Web-бетке кестені қосу",
        # Section 4.5 internal
        "Видеороликті кірістіру", "Музыка қосу",
        "Избранные фоны",
        # Supplementary material
        "«Сайттың мобильді нұсқасын құру»",
        # Section 4.6 internal
        "Сайтты тегін жариялау",
        # Section 4.7 internal
        "Сайтты насихаттау",
        "Мега тегтер", "Аутрич",
        "Сайтты ішкі оңтайландыру",
        "Сайтты сыртқы оңтайландыру",
        "Әлеуметтік желілер", "Сайттар каталогтары",
        # Section 5.1 internal
        # Section 5.2 internal
        "Мұғаліммен оқыту", "Мұғалімсіз оқыту",
        # Section 5.3 internal
        "Нейрондық желілердің құраушылары және олардың параметрлері",
        "Жасанды нейрон", "Тереңдетіп оқыту",
        # Section 5.4 internal
        "Блокчейннің қолданылуы",
        # Project work internal headings
        "Жобалау-зерттеу іс-әрекетінің кезеңдері:",
        "Жобалық жұмысқа арналған тақырыптар",
        "Міндеттері:", "Жобамен жұмыс істеу кезеңдері",
        # End-of-book
        "ГЛОССАРИЙ", "ПАЙДАЛАНЫЛҒАН ӘДЕБИЕТТЕР ТІЗІМІ",
        "ИНТЕРНЕТ-РЕСУРСТАР", "МАЗМҰНЫ", "Оқу басылымы",
        "ИБ №", "Users",
    }
    ...
    # Prefix matches
    internal_prefixes = [
        "Есте сақтаңдар", "Есте сактандар", "Есте сақтацдар", "Eсте сақтацдар",
        "І БӨЛІМ", "ІІ БӨЛІМ", "ІІІ БӨЛІМ", "IV БӨЛІМ", "V БӨЛІМ",
        "I БӨЛІМ", "II БӨЛІМ", "III БӨЛІМ",
        "Практикалық жұмыстар", "№1 практикалық", "№ 1 практикалық",
        "№2 практикалық", "№ 2 практикалық",
        "Тәжірибе жасаңдар",
        "Тереңдетіп оқуға",
        "«Чат-бот құру»",
        "Foogle:",
    ...
    # Headings containing specific keywords
    keyword_internals = [
        "OneDrive", "Яндекс", "SEO", "Google",
        "UI дизайнер", "WIX",
    ]
    for kw in keyword_internals:
        if kw in clean:
            return True
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    in_abstract = False
    in_stopped_section = False  # True when inside test/project section
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Global stop markers (glossary, bibliography, TOC)
        if RE_STOP.match(line):
            break

        # Test/project section stop — stop accumulating content
        if RE_SECTION_STOP.match(line):
            current_paragraph = None
            in_stopped_section = True
            continue
        ...
        # Detect standard paragraph heading: "## 1.1. Title", "### 3.3 Title"
        ...
        if m_std:
            ...
            if info:
                in_stopped_section = False
                # Use clean title from TOC if available
                known_title = None
                for ch in CHAPTERS:
            ...
            if current_paragraph is not None and not in_stopped_section:
                current_paragraph.content_lines.append(line)
            continue

        # In stopped section, skip everything until next paragraph heading
        if in_stopped_section:
            continue
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Информатика 10-сынып Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_38.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Информатика 10-сынып Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 10-сынып textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 10-сынып — Textbook Import")
    print("=" * 70)
