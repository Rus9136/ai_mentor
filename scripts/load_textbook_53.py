#!/usr/bin/env python3
"""
Load 'Биология 9-класс' (Russian) into the AI Mentor database.

Parses the Mathpix MMD file, creates chapter/paragraph records,
and creates ParagraphContent records for language='ru'.

This is a BIOLOGY textbook — no exercises, 16 глав, 61 paragraphs.
Chapters are determined by § number ranges from СОДЕРЖАНИЕ.
Has "Введение" before §1 (stored as paragraph 0 in chapter 1).
Chapters 1 and 6 have plain-text headings (no ##).
Chapter 9 (ДВИЖЕНИЕ) has no § paragraphs — only Демонстрация + Лаб.работа.

Usage:
    python scripts/load_textbook_53.py --dry-run           # Parse only
    python scripts/load_textbook_53.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_53.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 53  # Already exists in DB
TEXTBOOK_TITLE = "Биология 9-класс"
...
MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "53" / "textbook_53.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "53"

# Chapter mapping: § number ranges → chapter info (from СОДЕРЖАНИЕ)
# Chapter 9 has no § paragraphs (only Демонстрация + Лаб.работа)
CHAPTERS = [
    {"number": 1,  "title": "Глава 1. Клеточная биология",                                           "para_from": 0,  "para_to": 2},
    {"number": 2,  "title": "Глава 2. Разнообразие живых организмов. Биосфера и экосистемы",           "para_from": 3,  "para_to": 6},
    {"number": 3,  "title": "Глава 3. Влияние деятельности человека на окружающую среду",              "para_from": 7,  "para_to": 9},
    {"number": 4,  "title": "Глава 4. Питание",                                                       "para_from": 10, "para_to": 12},
    {"number": 5,  "title": "Глава 5. Транспорт веществ",                                             "para_from": 13, "para_to": 15},
    {"number": 6,  "title": "Глава 6. Дыхание",                                                       "para_from": 16, "para_to": 17},
    {"number": 7,  "title": "Глава 7. Выделение",                                                     "para_from": 18, "para_to": 21},
    {"number": 8,  "title": "Глава 8. Координация и регуляция",                                       "para_from": 22, "para_to": 28},
    # Chapter 9 (ДВИЖЕНИЕ) — no §, will be created but with no paragraphs
    {"number": 9,  "title": "Глава 9. Движение",                                                      "para_from": -1, "para_to": -1},
    {"number": 10, "title": "Глава 10. Молекулярная биология",                                        "para_from": 29, "para_to": 29},
    {"number": 11, "title": "Глава 11. Клеточный цикл",                                               "para_from": 30, "para_to": 32},
    {"number": 12, "title": "Глава 12. Закономерности наследственности и изменчивости",                "para_from": 33, "para_to": 43},
    {"number": 13, "title": "Глава 13. Микробиология и биотехнология",                                 "para_from": 44, "para_to": 44},
    {"number": 14, "title": "Глава 14. Размножение",                                                  "para_from": 45, "para_to": 49},
    {"number": 15, "title": "Глава 15. Рост и развитие",                                               "para_from": 50, "para_to": 51},
    {"number": 16, "title": "Глава 16. Эволюционное развитие",                                        "para_from": 52, "para_to": 61},
]
...
INTERNAL_HEADINGS = {
    ...
    "Анализ/Синтез",
    "Дискуссия",
    # Lab work and modeling headings (included in preceding §)
    "Лабораторная работа",
    "Моделирование",
    "Демонстрация",
    "Ход работы",
    # OCR-damaged chapter/section markers
    "Парииковые газы",  # OCR: "Парниковые газы"
    # Preamble/meta headings
    "Условные обозначения:",
    "Цель изучения темы",
    "Дорогие ребята!",
    # Author/book info in preamble that appears as ##
    "Н. Г. АСАНОВ, А. Р. СОЛОВЬЕВА, Б. Т. ИБРАИМОВА",
    "Учебник для 9 класса общеобразовательной школы",
    "Асанов Н. Г. и др.",
    "Учебное излание",
    "Асанов Нигметулла Гатауоннч",
    "БИОЛОГИЯ",
    # Watermarks
    "Все учебники Казахстана",
    "СОДЕРЖАНИЕ",
    "Список рекомендуемой литературы",
    # Lab work multiline titles
    "Работа основных мышц",
    "Продолжение",
    # Subtopic headings that appear inside §§
    "Естественный отбор, движущий и стабилизирующий.",
...
# Paragraph heading: "## § 1. Title", "## § 4．Title"
...
RE_STOP = re.compile(
    r"^#{1,2}\s+(КРАТКИЙ ТОЛКОВЫЙ СЛОВАРЬ\b|Список использованн\w+\b"
    r"|Список рекомендуемой\b|СОДЕРЖАНИЕ\b)",
    re.IGNORECASE,
)

# Глава heading — with or without ##
RE_CHAPTER = re.compile(
    r"^(#{1,2}\s+)?Глава\s+\d+", re.IGNORECASE
)
...
# §47 lost its heading during OCR — detect by "Цель изучения" + "менструальный цикл"
RE_PARA47_MARKER = re.compile(
    r"^>\s*Цель изучения этой темы:\s*описать менструальный цикл",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    # Глава headings — these are chapter markers, not paragraphs
    if RE_CHAPTER.match(clean) or RE_CHAPTER.match("## " + clean):
        return True

    # "Подумайте" / "Вспомните" type headings
    if clean.startswith("Подумайте") or clean.startswith("Вспомните"):
        return True
    ...
    # Lab work / Modeling / Demonstration headings that weren't caught above
    if clean.startswith("Лабораторн") or clean.startswith("Моделирован"):
        return True
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        # Special case: §47 lost its heading during OCR.
        # Detect by "Цель изучения этой темы: описать менструальный цикл"
        if RE_PARA47_MARKER.match(line):
            in_preamble = False
            current_paragraph = ParsedParagraph(
                title="Менструальный цикл. Роль гормонов эстрогена и прогестерона",
                number=47,
                raw_number="47",
            )
            paragraphs.append(current_paragraph)
        ...
        # Глава headings as plain text or ## — skip as chapter boundary markers
    ...
    # Return all chapters (including empty ones like Chapter 9)
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        f"-- Биология 9-класс Textbook Import (Russian)",
        f"-- Generated by load_textbook_53.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        "-- Биология 9-класс Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Биология 9-класс textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Биология 9-класс — Textbook Import")
    print("=" * 70)
