#!/usr/bin/env python3
"""
Load 'Информатика 7-класс (Атамура)' into the AI Mentor database.
...
Section 3.5 has a split heading (number and title on separate ## lines).

Usage:
    python scripts/load_textbook_58.py --dry-run           # Parse only
    python scripts/load_textbook_58.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_58.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 58
TEXTBOOK_TITLE = "Информатика 7-класс (Атамура)"
...
MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "58" / "textbook_58.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "58"

# Chapter mapping based on ОГЛАВЛЕНИЕ (TOC)
CHAPTERS = [
    {
        "number": 1,
        "title": "Раздел I. Измерение информации и компьютерная память",
        "paragraphs": [
            {"raw": "1.1", "title": "Единицы измерения информации"},
            {"raw": "1.2", "title": "Перевод из одних единиц измерения информации в другие"},
            {"raw": "1.3", "title": "Компьютерная память"},
            {"raw": "1.4", "title": "Размеры файлов"},
            {"raw": "1.5", "title": "Создание и распаковка архивов различных форматов"},
        ],
    },
    {
        "number": 2,
        "title": "Раздел II. Сети и безопасность",
        "paragraphs": [
            {"raw": "2.1", "title": "Компьютерные сети и их классификация"},
            {"raw": "2.2", "title": "Антивирусная безопасность"},
            {"raw": "2.3", "title": "Пользовательский интерфейс"},
        ],
    },
    {
        "number": 3,
        "title": "Раздел III. Решение задач с помощью электронных таблиц",
        "paragraphs": [
            {"raw": "3.1", "title": "Таблицы в текстовом процессоре"},
            {"raw": "3.2", "title": "Форматирование элементов электронных таблиц"},
            {"raw": "3.3", "title": "Типы данных"},
            {"raw": "3.4", "title": "Условное форматирование в электронной таблице"},
            {"raw": "3.5", "title": "Графическое представление табличных данных"},
            {"raw": "3.6", "title": "Моделирование процессов в электронных таблицах"},
        ],
    },
    {
        "number": 4,
        "title": "Раздел IV. Программирование на языке Python",
        "paragraphs": [
            {"raw": "4.1", "title": "Работа с файлами"},
            {"raw": "4.2", "title": "Программирование алгоритмов ветвления"},
            {"raw": "4.3", "title": "Программирование вложенных условий"},
            {"raw": "4.4", "title": "Программирование составных условий"},
            {"raw": "4.5", "title": "Организуем выбор"},
        ],
    },
    {
        "number": 5,
...
# Paragraph heading: "## 1.1. TITLE" or "## 3.5" (split heading, no title)
RE_PARA_HEADING = re.compile(
    r"^#{1,3}\s+(\d+\.\d+)\.?\s*(.*)"
)

# Test/assignment section heading
RE_TEST_SECTION = re.compile(
    r"^#{1,2}\s+(?:ТЕСТОВЫЕ\s+)?ЗАДАНИЯ\s+К\s+РАЗДЕЛУ", re.IGNORECASE
)

# Stop markers
RE_STOP = re.compile(
    r"^#{1,2}\s+(ГЛОССАРИЙ\b|СПИСОК ИСПОЛЬЗОВАННОЙ\b|ИНТЕРНЕТ-РЕСУРСЫ\b)",
    re.IGNORECASE,
)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    for raw_line in lines:
        ...
        # Test/assignment sections: skip until next paragraph
        ...
        # If in test section, skip content
        if in_test_section:
            continue
...
def md_lines_to_html(lines: list[str], textbook_id: int) -> str:
    ...
    for raw_line in content.split("\n"):
        ...
        # Letter-prefixed answer: "a) text", "б) text", "в) text", "г) text", "д) text"
        letter_match = re.match(r"^([a-zа-яё])\)\s+(.+)", line.strip())
        if letter_match:
            flush_paragraph()
            html_parts.append(
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Информатика 7-класс (Атамура) Textbook Import (Russian)",
        f"-- Generated by load_textbook_58.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Информатика 7-класс (Атамура) Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Информатика 7-класс (Атамура) textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Информатика 7-класс (Атамура) — Textbook Import")
    print("=" * 70)
