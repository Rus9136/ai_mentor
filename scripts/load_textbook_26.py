#!/usr/bin/env python3
"""
Load 'Қазақстан тарихы 8' (History of Kazakhstan, 1900-1945) into the AI Mentor database.
...
Has explicit бөлім (chapter) headings and § paragraphs.
Some § headings are plain text (not markdown headings).
Uses fullwidth characters in some places (OCR artifact).

Usage:
    python scripts/load_textbook_26.py --dry-run           # Parse only
    python scripts/load_textbook_26.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_26.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
GRADE_LEVEL = 8
AUTHORS = "К.Ускембаев, З.Сақтағанова, Г.Мұхтарұлы"
PUBLISHER = "Мектеп"
YEAR = 2025
ISBN = "978-601-07-1776-3"
LANGUAGE = "kk"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "26" / "textbook_26.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "26"

# Chapter mapping: § number ranges → chapter info (from TOC / Мазмұны)
CHAPTERS = [
    {
        "number": 1,
        "title": "І бөлім. XX ҒАСЫРДЫҢ БАСЫНДАҒЫ ҚАЗАҚСТАН",
        "para_from": 1,
        "para_to": 14,
    },
    {
        "number": 2,
        "title": "ІІ бөлім. ҚАЗАҚСТАНДА КЕҢЕСТІК БІЛІКТІҢ ОРНЫҒУЫ",
        "para_from": 15,
        "para_to": 28,
    },
    {
        "number": 3,
        "title": "ІІІ бөлім. ҚАЗАҚСТАН ТОТАЛИТАРЛЫҚ ЖҮЙЕ КЕЗЕҢІНДЕ",
        "para_from": 29,
        "para_to": 38,
    },
    {
        "number": 4,
        "title": "IV бөлім. КЕҢЕСТІК ҚАЗАҚСТАННЫҢ МӘДЕНИЕТІ: БІЛІМ МЕН ҒЫЛЫМ",
        "para_from": 39,
        "para_to": 46,
    },
    {
        "number": 5,
        "title": "V бөлім. КЕҢЕСТІК ҚАЗАҚСТАННЫҢ МӘДЕНИЕТІ: ӘДЕБИЕТ ПЕН ӨНЕР",
        "para_from": 47,
        "para_to": 49,
    },
    {
        "number": 6,
        "title": "VI бөлім. ҚАЗАҚСТАН ҰЛЫ ОТАН СОҒЫСЫ ЖЫЛДАРЫНДА",
        "para_from": 50,
        "para_to": 60,
    },
]
...
INTERNAL_HEADINGS = {
    ...
    "Сүрақтарға жауап беріндер", "Сүрақтарға жауап беріңдер",
    "Тапсырмалар", "Тапсырма",
    "Кестемен жұмыс", "Картамен жұмыс",
    "Нұсқаулық:", "Нүсқаулық:",
...
OCR_FIXES = {
    ...
    "кецестік": "кеңестік",
    "Үлы": "Ұлы",
    "кезеціндегі": "кезеңіндегі",
    "қажырлы ецбегі": "қажырлы еңбегі",
    "қазақстандықтардыц": "қазақстандықтардың",
}
...
class ParsedParagraph:
    title: str
    number: int       # First § number (e.g., 5 for "§ 5-6")
    raw_number: str   # Original string (e.g., "5-6")
...
RE_STOP = re.compile(
    r"^#{0,2}\s*(Маңызды оқиғалардың хронологиялық|Пайдаланылған әдебиеттер|Мазмұны)",
    re.IGNORECASE,
)
...
def is_internal_heading(heading: str) -> bool:
    ...
    # бөлім/белім headings (chapter-level, handled by TOC mapping)
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        f"-- Қазақстан тарихы 8 Textbook Import (Kazakh)",
        f"-- Generated by load_textbook_26.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Қазақстан тарихы 8 Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Қазақстан тарихы 8 textbook into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Қазақстан тарихы 8 (1900-1945) — Textbook Import")
    print("=" * 70)
