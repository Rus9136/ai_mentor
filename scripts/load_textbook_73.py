#!/usr/bin/env python3
"""
Load 'Русская литература 8-класс часть 1' into the AI Mentor database.

Parses the Mathpix MMD file, creates chapter/paragraph records,
and creates ParagraphContent records for language='ru'.

This is a LITERATURE textbook — no exercises, 2 раздела, 8 тем.
...
Usage:
    python scripts/load_textbook_73.py --dry-run           # Parse only
    python scripts/load_textbook_73.py --generate-sql FILE  # Generate SQL
    python scripts/load_textbook_73.py --update-content FILE # Update SQL
"""
import re
import sys
import hashlib
...
TEXTBOOK_ID = 73  # Already exists in DB
TEXTBOOK_TITLE = "Русская литература 8-класс часть 1"
SUBJECT_CODE = "russian_lit"
GRADE_LEVEL = 8
AUTHORS = "Шашкина Г.З., Анищенко О., Шмельцер В."
PUBLISHER = "Мектеп"
YEAR = 2019
ISBN = "978-601-07-0972-0"
LANGUAGE = "ru"

MD_FILE = PROJECT_ROOT / "uploads" / "textbook-mmd" / "73" / "textbook_73.mmd"
IMAGES_SRC_DIR = PROJECT_ROOT / "uploads" / "textbook-images" / "73"
...
TEMA_TITLES = {
    ...
    3: "Сакен Нурмакович Жунусов",
    4: "Виктор Петрович Астафьев",
    5: "Уильям Шекспир",
    6: "Александр Сергеевич Пушкин",
    7: "Лирика поэтов XIX века",
    8: "Олжас Омарович Сулейменов",
}
...
FIRST_TEMA_IN_CHAPTER = {1: 1, 2: 5}

# Chapter title headings to skip (they appear right after ## Раздел)
CHAPTER_TITLE_HEADINGS = {"ДЕТИ И ВЗРОСЛЫЕ", "ЛЮБОВЬ И ЧЕСТЬ"}
...
# Тема heading — handles OCR Latin/Cyrillic mix: Tema, Тема, Tема, etc.
# Requires digit after "Тема" to avoid matching "ТЕМА ЛЮБВИ В РУССКОЙ..."
RE_TEMA = re.compile(
    r"^#{1,2}\s+[ТTт][еeе][мmм][аaа]\s+(\d+)\.?\s*(.*)",
    re.IGNORECASE,
)

# Раздел (chapter) heading
RE_RAZDEL = re.compile(
    r"^#{1,2}\s+Раздел\s+(I{1,3}V?|V?I{0,3})\b",
    re.IGNORECASE,
)

# Stop markers — glossary, TOC, publisher info
RE_STOP = re.compile(
    r"^#{1,2}\s+(КРАТКИЙ\s+СЛОВАРЬ\b|Содержание\b|回湿回)",
    re.IGNORECASE,
)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    State machine:
    - SKIP: before first Раздел (foreword, title page)
    - CHAPTER_INTRO: after Раздел heading, before first Тема of that chapter
    - IN_PARAGRAPH: inside a Тема's content
    """
    ...
    intro_buffer: list[str] = []    # buffer for chapter intro content
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if m:
            ...
            intro_buffer = []
            # Don't create paragraph yet — wait for first Тема
        ...
        if m:
            ...
            # If this is the first tema in the chapter, prepend intro buffer
            if tema_num == FIRST_TEMA_IN_CHAPTER.get(ch_num):
        ...
        if current_chapter_num is not None and current_paragraph is None:
            # Skip chapter title headings (## ДЕТИ И ВЗРОСЛЫЕ, ## ЛЮБОВЬ И ЧЕСТЬ)
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        f"-- Русская литература 8-класс часть 1 — Textbook Import",
        f"-- Generated by load_textbook_73.py",
        "",
        "BEGIN;",
        "",
    ]
...
def generate_update_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    sql_lines = [
        "-- Русская литература 8-класс часть 1 — Content UPDATE",
...
def main():
    parser = argparse.ArgumentParser(
        description="Load Русская литература 8-класс часть 1 into AI Mentor database"
    )
    parser.add_argument(
    ...
    print("=" * 70)
    print("  Русская литература 8-класс часть 1 — Textbook Import")
    print("=" * 70)
