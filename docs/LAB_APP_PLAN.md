...
### Фаза 3. Карта истории Казахстана — MVP модуль — ВЫПОЛНЕНО (20.03.2026)

**Цель:** полноценная интерактивная карта с 7 историческими эпохами.

**Что сделано:**

1. GeoJSON данные (2 файла):
   - `lab-app/public/data/history/territories.geojson` — полигоны территорий 7 эпох (Саки → Казахская ССР и независимость), с цветовой кодировкой и fillOpacity
   - `lab-app/public/data/history/markers.geojson` — 17 маркеров: 7 городов (Туркестан, Отрар, Сыгнак, Алматы, Астана, Тараз, Шымкент), 4 битвы (Таласская, Орбулак, Аныракай, Сауран), 1 археология (курган Иссык), 3 точки Шёлкового пути + LineString маршрут, 1 географический (Аральское море), 1 современный (Байконур)
   - Все маркеры с двуязычными описаниями (ru/kk) и привязкой к эпохам через `epoch_ids`

2. Новые компоненты:
   - `TerritoryLayer.tsx` — GeoJSON полигоны, фильтрация по epoch_id, стилизация из GeoJSON properties
   - `MapMarkers.tsx` — CircleMarker с popup, фильтрация по epoch_ids, LineString для Шёлкового пути, 7 типов маркеров с разными стилями (city, capital, battle, archaeological, silk_road, geographic, modern)

3. Обновлённые компоненты:
   - `epochs-data.ts` — 7 эпох с epoch_id, name_kk, description_kk, capital, key_events
   - `HistoryMap.tsx` — интеграция TerritoryLayer + MapMarkers, динамическая загрузка GeoJSON
   - `EpochInfo.tsx` — двуязычная боковая панель со столицей, ключевыми событиями (Landmark, ScrollText иконки)
   - `HistoryLab.tsx` — загрузка GeoJSON через fetch, передача locale во все дочерние компоненты
   - `Timeline.tsx` — двуязычные метки эпох (name_kk), локализованные подписи дат
   - `types/lab.ts` — EpochData расширен: epoch_id, name_kk, description_kk, capital, key_events

4. Фикс UX:
   - Неавторизованные пользователи перенаправляются на `/login` (раньше видели пустую страницу)

**Не реализовано (отложено):**
- Редактирование карты через админку (GeoJSON хранится статически в репо)
- Привязка эпох к paragraph_id учебника (все `null`, структура готова)
- Zoom к маркеру при клике на событие в sidebar
...
### Фаза 6. Химия — 3D молекулы — ВЫПОЛНЕНО (20.03.2026)

**Цель:** второй предметный модуль — интерактивные 3D-модели молекул.

**Что сделано:**

1. Установлен `3dmol` (npm) — WebGL рендер молекул, dynamic import для Next.js SSR-совместимости

2. Компоненты (`lab-app/src/components/chemistry/`):
   - `ChemistryLab.tsx` — главный контейнер (viewer + selector + controls + sidebar)
   - `MoleculeViewer.tsx` — 3Dmol.js интеграция, загрузка SDF из PubChem REST API по CID
   - `MoleculeInfo.tsx` — боковая панель: формула, геометрия, тип связи, факты
   - `molecules-data.ts` — 8 молекул с двуязычными описаниями (ru/kk)

3. Молекулы (8 шт.):
   - H₂O (вода), CO₂ (углекислый газ), CH₄ (метан), NH₃ (аммиак)
   - C₂H₅OH (этанол), C₆H₆ (бензол), C₆H₁₂O₆ (глюкоза), H₂SO₄ (серная кислота)
   - Каждая с PubChem CID, описанием, типом связи, геометрией, интересными фактами

4. Функциональность:
   - 4 стиля отображения: шарики+палочки, палочки, сферы (Ван-дер-Ваальс), каркас
   - Авто-вращение (toggle)
   - Горизонтальный скроллируемый селектор молекул внизу
   - Полноэкранный 3D viewer
   - Двуязычный UI (ru/kk)

5. Инфраструктура:
   - Маршрут: `/lab/2` → ChemistryLab (dynamic import, ssr: false)
   - Каталог: карточка «Химия — Молекулы» активирована (available: true)
   - БД: lab id=2, subject_id=27 (Химия), lab_type='molecule_3d'

**Не реализовано (отложено):**
- Задания на молекулах («Найди атом», «Посчитай связи»)
- Привязка к параграфам учебника химии
- Оффлайн-режим (сейчас SDF загружается из PubChem API)
