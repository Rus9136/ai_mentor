#!/usr/bin/env python3
"""
Load 'Алгебра және анализ бастамалары 10 сынып, 1-бөлім' (Kazakh) into AI Mentor DB.

Adapted from load_algebra9_textbook.py for LaTeX \\section*{} format.
The source MD file uses \\section*{} instead of markdown ## headings.

Usage:
    python load_algebra10_textbook.py --dry-run
    python load_algebra10_textbook.py --generate-sql /tmp/algebra10_import.sql
    python load_algebra10_textbook.py --exercises /tmp/algebra10_exercises.sql
"""
import re
import sys
import os
...
TEXTBOOK_TITLE = "Алгебра және анализ бастамалары 10 сынып, 1-бөлім"
...
MD_FILE = SCRIPT_DIR / "489_with_local_images.md"
...
CHAPTER_DEFS = [
    (0, "7-9-сыныптардағы алгебра курсын қайталауға арналған жаттығулар"),
    (1, "Функция, оның қасиеттері және графигі"),
    (2, "Тригонометриялық функциялар, олардың қасиеттері және графиктері"),
    (3, "Кері тригонометриялық функциялар"),
    (4, "Тригонометриялық теңдеулер және теңсіздіктер"),
    (5, "Комбинаторика элементтері және ықтималдықтар теориясы"),
]
...
RE_CHAPTER_TITLE = re.compile(
    r"^(?:"
    r"\d+[-\s]*тарау"                          # "1-тарау", "2-тарау ."
    r"|ФУНКЦИЯ,\s*ОНЫ[ҢН]"                    # chapter 1 title
    r"|ТРИГОНОМЕТРИЯЛЫҚ ФУНКЦИЯЛАР"            # chapter 2 title
    r"|КЕРІ ТРИГОНОМЕТРИЯЛЫҚ"                  # chapter 3 title
    r"|ТРИГОНОМЕТРИЯЛЫҚ ТЕҢДЕУЛЕР"             # chapter 4 title
    r"|КОМБИНАТОРИКА"                          # chapter 5 title
    r"|\d+$"                                    # standalone numbers "4", "5"
    r"|7-9-сыныптард"                          # answer subsection header
    r")",
    re.IGNORECASE,
)
...
def parse_textbook(md_path: Path) -> list[ParsedChapter]:
    ...
    phase = "preamble"  # preamble | review | content | answers
    in_figure = False
    ...
    for line_num, raw_line in enumerate(lines, 1):
        ...
        if para_match:
            ...
            if phase == "preamble" or phase == "review":
                phase = "content"
            continue

        # ── Detect review section start ──────────────────────────────
        if phase == "preamble":
            if section_content and RE_REVIEW_START.search(section_content):
                phase = "review"
                current_paragraph = ParsedParagraph(
                    title="Қайталауға арналған жаттығулар",
                    number=0,
                )
                all_paragraphs.append(current_paragraph)
                continue
...
def find_or_create_textbook(session, subject_id: int) -> int:
    ...
    result = session.execute(
        ...
        ),
        {
            ...
            "year": YEAR,
            "isbn": ISBN,
            "description": "Жалпы білім беретін мектептің жаратылыстану-математика бағытындағы 10-сыныбына арналған оқулық. 1-бөлім.",
        },
    )
...
def parse_answers_section(md_path: Path) -> dict[str, str]:
    ...
    for raw_line in lines:
        ...
        # Skip \section* subsection headers within answers
        if section_content:
            continue
...
def generate_sql(chapters: list[ParsedChapter], output_path: Path):
    ...
    lines = [
        "-- Algebra 10 Textbook Import (Kazakh)",
        ...
        f"    {escape_sql('Жалпы білім беретін мектептің жаратылыстану-математика бағытындағы 10-сыныбына арналған оқулық. 1-бөлім.')},",
        "    true, false, 1, false",
        "WHERE NOT EXISTS (",
...
def main():
    ...
    print("=" * 70)
    print("  Algebra 10 Textbook Import (Kazakh)")
    print("=" * 70)
